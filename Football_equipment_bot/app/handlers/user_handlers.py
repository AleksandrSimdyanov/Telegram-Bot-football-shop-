...
async def start_cmd(message: Message):
    ...
    await message.answer("Добро пожаловать в наш бот. Здесь вы сможете выбрать аммуницию\n"
...
async def indicate_product(call: CallbackQuery):
    product_id = call.data.split("_")[1]
    await call.message.edit_reply_markup(reply_markup=await kb.size_kb(product_id))

@user_router.callback_query(F.data.startswith("buy_"))
async def save_to_cart(call: CallbackQuery):
    _, product_id, size = call.data.split("_")
    user_id = call.from_user.id
    count = await rq.add_cart(user_id, product_id, size=size)
    await call.answer("Корзина обновлена")
    await call.message.edit_reply_markup(reply_markup=await kb.new_buy_kb(product_id, user_id, count, size))
...
async def make_minus(call: CallbackQuery):
    _, product_id, size = call.data.split("_")
    user_id = call.from_user.id
    count = await rq.remove_cart(user_id, product_id, size=size)
    if count > 0:
        await call.message.edit_reply_markup(reply_markup=await kb.new_buy_kb(product_id, user_id, count, size))
    elif count == 0:
...
async def show_order(call: CallbackQuery):
    ...
    for item in order_items:
        message_text += f"{item.name} - {item.quantity} шт. * {item.price} руб. (Размер: {item.size})\n"
