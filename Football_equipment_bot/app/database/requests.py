from app.database.models import Type, async_session, Product, Cart, Favorite, Order, OrderCart, Appeal, User
...
async def add_cart(user_id, product_id, quantity=1, size="1"):
    async with async_session() as session:
        cart = await session.scalar(select(Cart).where(Cart.user_id == user_id,
                                                       ...
                                                       Cart.status == "new"))
        if cart is None:
            cart = Cart(user_id=user_id, product_id=product_id, quantity=quantity, size=size)
            session.add(cart)
        else:
            cart.quantity += 1
...
async def remove_cart(user_id, product_id, size):
    async with async_session() as session:
        cart = await session.scalar(select(Cart).where(Cart.user_id == user_id,
                                                       Cart.product_id == product_id,
                                                       Cart.size == size,
...
async def get_favorites_by_user_id(user_id):
    async with async_session() as session:
        fav = await session.scalars(select(Favorite).where(Favorite.user_id == user_id))
    return fav
